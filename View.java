...
import java2.java2_oop.Unit.Coords;
...
public class View {
    ...
    private static String getChar(int x, int y){
        String out = "| ";
        for (Man man: Main.teams) {
            if (man.getCoords()[0] == x && man.getCoords()[1] == y){
                if (man.getHp() == 0) {
                    out = "|" + (AnsiColors.ANSI_RED + man.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                    break;
                }
                if (Main.team1.contains(man)) out = "|" + (AnsiColors.ANSI_GREEN + man.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                if (Main.team2.contains(man)) out = "|" + (AnsiColors.ANSI_BLUE + man.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                break;
            }
        }
        return out;
